...
class ItemFeaturePage extends StatefulWidget {
  final Map<String, dynamic> product;

  const ItemFeaturePage({super.key, required this.product});

  @override
  State<ItemFeaturePage> createState() => _ItemFeaturePageState();
}
...
class _ItemFeaturePageState extends State<ItemFeaturePage> {
  ...
  /*Future<void> _fetchreviews() async {
    try {
      ...
      if (sellerDoc.exists) {
        final reviewsSnapshot = await sellerDoc.reference
            .collection('products')
            .doc(widget.product['productID'])
            .collection('review')
            .get();
        ...
        for (var doc in reviewsSnapshot.docs) {
          ...
          totalreview += double.parse(data['review'].toString());
        }
                  ...
                  return GestureDetector(
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              ItemFeaturePage(product: product),
                        ),
                      );
                    },
                    child: Container(
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // review Section
                      // In your existing build method, after _buildreviewSection()
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        _addToCart(widget.product,
                            widget.product['type'] ?? 'defaultType');
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: const Color(0xFF808569),
                        foregroundColor: Colors.white,
