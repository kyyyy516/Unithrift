...
class _ProductUploadPageState extends State<ProductUploadPage> {
  ...
  bool _validateProductDetails() {
    ...
    if (_selectedCondition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select the product condition.')),
      );
      return false;
    }
  ...
  Future<void> _uploadProduct() async {
    ...
    setState(() {
      ...
      final productData = {
        ...
        'condition': _selectedCondition, // New condition field
        'imageUrls': _imageUrls, // Store multiple image URLs
      ...
      // Save to Firestore under user's products collection
      final currentUserRef = FirebaseFirestore.instance.collection('users').doc(currentUser.uid);
      await currentUserRef.collection('products').add(productData);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("What's your item?", 
                  style: TextStyle(
